...
<!------------------------------------------- PUBLIC ------------------------------------------->	
	...
	<cffunction name="renderAvatar" output="false" access="public" returntype="any" hint="Render an avatar image">
		...
		<cfif rc.codexoptions.wiki_gravatar_display>
			...
			<cfoutput>
			<img align="middle" src="http://www.gravatar.com/avatar.php?gravatar_id=#lcase(Hash(email))#&r=#rc.CodexOptions.wiki_gravatar_rating#&s=#arguments.size#" alt="#rc.oUser.getUsername()#'s Gravatar" />
			</cfoutput>
			</cfsavecontent>
		</cfif>
